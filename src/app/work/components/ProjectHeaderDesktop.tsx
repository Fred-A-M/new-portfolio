...
function useAnimatedParagraph(scrollYProgress: MotionValue<number>, index: number, total: number) {
  ...
  const y = useTransform(scrollYProgress, [startPoint, endPoint], ['100vh', '0vh']);
...
function AnimatedParagraph({
  ...
  return (
    <motion.p style={{ y }} className="origin-left transition-opacity">
      {text}
    </motion.p>
  );
}
...
export default function ProjectHeaderDesktop({ project }: ProjectHeaderProps) {
  ...
  return (
    <div className="relative h-[250vh]" ref={headerRef}>
      {' '}
      ...
      <div className="sticky top-24 h-[75vh] grid grid-cols-2">
        ...
        <div className="flex flex-col gap-2 justify-center sticky-text-wrapper">
          ...
          <motion.div className="text-block sm:text-4xl text-xl" style={{ y: 0 }}>
            <p className="radio-canada-big-bold">{project.name}</p>
            <p>{project.client}</p>
          </motion.div>

          {/* Description container */}
          <motion.div className="text-block-description text-lg flex flex-col gap-2">
