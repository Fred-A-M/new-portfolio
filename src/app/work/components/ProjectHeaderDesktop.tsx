  ...
  return (
    <>
      <div className="relative h-[300vh] flex justify-center" ref={headerRef}>
        ...
        <div className="sticky top-24 h-[75vh] grid grid-cols-2 lg:w-[90%] w-full mx-auto">
          {/* Left: scrollable title + description */}
          <div className="relative h-full flex lg:justify-center justify-start">
            {/* Title section – now absolutely centered */}
            <motion.div
              className="sm:text-4xl text-xl absolute top-1/2 -translate-y-1/2"
              style={{ y: yTitle }}
            >
              <p className="rag-normal">{project.name}</p>
            </motion.div>
      ...
      <div className="lg:w-[90%] w-full mx-auto grid grid-cols-2">
        <div className="lg:col-span-1 col-span-2 text-xl">
          {project.description}
        </div>
      </div>
    </>
